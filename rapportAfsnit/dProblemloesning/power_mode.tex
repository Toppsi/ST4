...
\begin{table}[H]
	...
	\end{tabular}%
	\caption{I tabellen ses strømforbruget samt opvågningstiden for PSoC 4200M, som er den valgte MCUs target processor. (Modificeret) \citep{Semiconductor2016PowerMode}}
	\label{tab:Powermode}
...
Under deep-sleep er højfrekvente clocks deaktive, og da ADCen kræver sådanne clocks, er denne ligeledes inaktiv under denne tilstand. Deep-sleep mode kan benyttes, hvis høj performance i analoge eller digitale enheder ikke kræves for funktionen. Tilstanden deep-sleep kræver specifikke interrupts for at vågne, som for eksempel et I$^{2}$C eller GPIO interrupt, da disse operationssystemer er aktive og fortsat fungerer som slave. \citep{Semiconductor2016PowerMode} \\
