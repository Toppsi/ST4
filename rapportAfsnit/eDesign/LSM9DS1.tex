...
Sensorens opbygning fremgår af \figref{fig:IC_pins}, hvor der er henholdsvis fire pins på venstre side og 9 pins på højre side. De fire pins på venstre side af ICen vil blive benyttet til spændingstilkobling samt til at læse sensorens outputdata fra. GND og VDD er pins til henholdsvis jord og spændingsforsyning, mens SDA er I$^2$C datapin, hvor data bliver sendt og modtaget, og SCL er en seriel clock, der blandt andet sørger for synkron dataopsamling.
\begin{figure}[H]
	\centering
...
Gyroskopet skal samples med mindst 60~Hz, men ifølge gyroskopets datablad kan den indstilles til 59,5~Hz eller 119~Hz, hvorfor denne vælges til en samplingsfrekvens 119~Hz.
...
- BENYTTER REGISTER HVORAF HEXKODEN FREMGÅR --> DENNE HEXKODE SENDES TIL IC FOR AT INDSTILLE TIL ØNSKEDE SAMPLINGSFREKVENS OG ARBEJDSOMRÅDE
- MODSTANDE SOM SØRGER FOR AT KUNNE HENTE DATA FRA IC (SE DATABLAD)
