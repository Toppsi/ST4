...
Implementeringen af algoritmen for detektering af gang og løb tager udgangspunkt i data fra pilotforsøget, og de designmæssige aspekter som beskrevet i \secref{design_algo_g_l}. Algoritmen er derfor designet og implementeret som det ses på \figref{fig:basic_algo_g_l}.
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{figures/cDesign/Algoritme_g_l_basic.png}
...
Algoritmen henter low byte og high byte fra ICens outputdata, hvorefter de enkelte samples gennemgår en signalbehandling. Denne signalbehandling og den tilhørende C kode vil blive yderligere forklaret i \figref{fig:signalbehandling_g_l}. Efter samples er blevet behandlet, bliver time_counteren startet således denne tæller antallet af samples mellem en given event i algoritmen. De behandlede samples gemmes i variabel ’Value[0]’, hvortil den foregående behandlede sample bliver gemt i variabel ’Value[1]’. Denne måde hvorpå to samples gemmes i hver sin variabel muliggør blandt andet, at algoritmen kan undersøge disse variablers størrelse i forhold til en tærskelværdi. \\
Efter den nye og den foregående sample er placeret i separate variabler, er det muligt at benytte tærskelværdier til at detektere hvorvidt der er ingen aktivitet, gang eller løb. Disse elementer af algoritmen forklares yderligere i \figref{fig:ingen_ak_pseudo}, \figref{fig:gang_pseudo} og \figref{fig:loeb_pseudo}. Tærskelværdierne for de pågældende fysiske aktiviteter er bestemt i \secref{design_algo_g_l}. \\ 
Yderligere bliver den maksimale peak bestemt. Dette udføres ved at undersøge hvorvidt en ny sample er større end den forrige på en stigende del af en kurve. Når kurven derimod er faldende, vil detektering af maksimale peak ikke udføres.

Algoritmens signalbehandling involverer fire elementer; elliptisk filtrering, dividering, kvadrering og en moving average filtrering. Dette fremgår desuden af \figref{fig:signalbehandling_g_l}.
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{figures/cDesign/Signalbehandling_gl_pesudo.png}
	\caption{På figuren ses algoritmens signalbehandling i C kode, beskrevet som pseudokode.}
	\label{fig:signalbehandling_g_l}
\end{figure} \vspace{-0.5cm}
Efter datamodtagelse fra ICen bliver samples IIR filtreret, hvoraf givne ’a’ og ’b’ koefficienter for et elliptisk filter, benyttes til at behandle samples med. Efterfølgende retureneres de behandlede samples, hvormed disse er klar til næste del af signalbehandlingen. De pågældende koefficienter bestemmes i MATLAB, hvor filterets knækfrekvenser og dæmpning benyttes til at bestemme det elliptiske filters koefficienter. Disse koefficienter skriver i C koden for algoritmen, således filtreringen er med henhold til det ønskede filter designet i MATLAB.\\
De enkelte samples bliver herefter bitskiftet med 5, hvilket svarer til at dividere med 32. Efterfølgende bliver samples kvadreret, ved at gange hver sample med sig selv. Afslutningsvis filtreres samples med et moving average filter, hvilket medfører en blødere kurve, hvilket fremgår af \figref{fig:algoritme_cykling2}.

Algoritmen er designet og implementeret således denne kan detektere, hvis brugeren ikke er fysisk aktiv. Denne del af algoritmen fremgår af \figref{fig:ingen_ak_pseudo}.
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{figures/cDesign/ingen_aktivitet_gl_pseudo.png}
	\caption{På figuren ses algoritmens metode til at detektere hvis brugeren ikke er fysisk aktiv. Det fremgår, at hvis time_count er større end tre sekunder sampling, 1428 samples, da detekteres brugeren som værende ikke fysisk aktiv.}
	\label{fig:ingen_ak_pseudo}
\end{figure} \vspace{-0.5cm}
Algoritmen detekterer at brugeren ikke er fysisk aktiv, i tilfælde af at time counteren opnår et antal samples som er større end tre sekunders sampling. Hvis dette er tilfældet, bliver de pågældende variabler nulstillet.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{figures/cDesign/gang_ckode_pseudo.png}
...
\end{figure} \vspace{-0.5cm}
Den venstre del af figuren er ansvarlig for optælling af skridt, hvortil den højre del bestemmer varigheden mellem hælnedslag ved gang. \\
Førend algoritmen kan tælle op på antal skridt for gang, da skal amplituden for ’Value[0]’ være i intervallet mellem tærskelværdierne for gang og løb. Yderligere skal den foregående sample, ’Value[1]’, have en størrelse som er lavere end tærskelværdien for gang. Hvis disse tre kriterier er opfyldt, da indstilles ’Trace_Max’ til at være lig 1. Dette tillader, at algoritmen begynder at finde den maksimale peak, som illustreret på \figref{fig:basic_algo_g_l}. Ydermere bliver ’Step_count_gang’ talt op, indtil denne variabel er maksimalt 3. \\
Højre del af \figref{fig:gang_pseudo} repræsenterer den del af algoritmen som finder varigheden mellem hælnedslag og sender dette gennem bluetooth. Førend denne del af algoritmen bliver udført, da skal ’Value[0]’ være mindre end tærskelværdien for gang og ’Value[1]’ skal være over denne tærkselværdien. Yderligere skal det bestemte maksimale peak være mindre end tærskelværdien for løb. Hvis kriterierne opfyldes, da bliver det maksimale peak gemt i ’PEAK’, samt algoritmen indstilles således denne ikke længere finder et maksimalt peak. Yderligere er det gældende, at hvis der er detekteret ét hælnedslag for gang, da bliver henholdsvis time counteren og ’PEAK’ nulstillet. \\
For at bestemme varigheden mellem hælnedslag, da skal antal hælnedslag for gang overstige værdien 1. Under disse omstændigheder, bliver ’time_count’ og ’PEAK’ bitskiftet hvorefter disse værdier sendes over bluetooth. Afslutningsvis nulstilles time counteren.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{figures/cDesign/loeb_ckode_pseudo.png}
...
\end{figure} \vspace{-0.5cm}
Den venstre del af figuren er ansvarlig for optælling af skridt, hvortil den højre del bestemmer varigheden mellem hælnedslag ved løb. \\
Førend algoritmen kan tælle op på antal skridt for løb, da skal amplituden for ’Value[0]’ være større end tærskelværdien for løb. Yderligere skal den foregående sample, ’Value[1]’, have en størrelse som er lavere end tærskelværdien for løb. Hvis disse to kriterier er opfyldt, da indstilles ’Trace_Max’ til at være lig 1. Dette tillader, at algoritmen begynder at finde den maksimale peak, som illustreret på \figref{fig:basic_algo_g_l}. Ydermere bliver ’Step_count_løb’ talt op, indtil denne variabel er maksimalt 3. \\
Højre del af \figref{fig:loeb_pseudo} repræsenterer den del af algoritmen som finder varigheden mellem hælnedslag og sender dette gennem bluetooth. Førend denne del af algoritmen bliver udført, da skal ’Value[0]’ være mindre end tærskelværdien for løb og ’Value[1]’ skal være over denne tærkselværdien. Hvis kriteriet opfyldes, da bliver det maksimale peak gemt i ’PEAK’, samt algoritmen indstilles således denne ikke længere finder et maksimalt peak. Yderligere er det gældende, at hvis der er detekteret ét hælnedslag for løb, da bliver henholdsvis time counteren og ’PEAK’ nulstillet. \\
For at bestemme varigheden mellem hælnedslag, da skal antal hælnedslag for løb overstige værdien 1. Under disse omstændigheder, bliver ’time_count’ og ’PEAK’ bitskiftet hvorefter disse værdier sendes over bluetooth. Afslutningsvis nulstilles time counteren.
