...
Implementeringen af algoritme til detektering af cykling opdeles i to deleelementer: signalbehandling og signalgenkendelse. Dette implementeres i MATLAB, ligeledes med brugerfladen.\\
Signalbehandlingens første trin består som tidligere nævnt, af en FFT over fire sekunders sampling. Måden hvorpå dette er gjort i MATLAB er ved at inputsignalet gemmes i et array mens samples tælles for et fire sekunders interval, når datalængden er fire sekunder, signalbehandles arrayet. 
Behandlingen af arrayet er en FFT, som i MATLAB implementeres ved først at centrere signalet omkring 0 ved hjælp af en detrend funktion, hvorefter MATLABs FFT funktion benyttes på det detrendede signal, afslutningsvist kvadreres den absolutte værdi af det FFT behandlede data.\fxnote{MATLAB kommando: $fft_z = abs(fft(detrend(input_data))).^2$} 
%Denne signalbehandling af et signal fra pilotforsøget er illustreret på \figref{fig:gyro_behandling}.  
\begin{figure}[H]
	\centering
...
Data fra pilotforsøget for forsøgsperson 2 er behandlet med ovenstående signalbehandling. Dette er gjort med henblik på fastsættelse af en tærskelværdi som adskiller cykling fra gang og løb.\\ 
På den øverste graf på \figref{fig:gyro_behandling} ses de rå gyroskopsignaler for gang, løb og cykling, hvoraf cykling tilnærmelsesvis ligner en sinus, hvorfor dennes energi er centreret omkring en frekvens efter signalbehandlingen, som det er illustreret på den nederste graf på \figref{fig:gyro_behandling}. Hertil ses det også at energien for gang og løb ikke centrerer sig omkring en enkelt frekvens, men om forskellige frekvenser, hvorfor spredningen af energi kan benyttes til signalgenkendelse. 

Efter signalbehandlingen implementeres signalgenkendelsen som består af, først at finde den maksimale magnitude og den tilhørende frekvens, resultatet heraf er aktivitetens dominerende frekvens. Herefter summeres værdierne for den maksimale magnitude, ved at summere intervallet $\pm$1~Hz omkring frekvens heraf. Hertil summeres også værdierne for FFTen fra 1 til 20~Hz. Afslutningsvist bestemmes hvor stor en procentdel summeringen for den maksimale magnitude udgør af summeringen for 1 til 20~Hz. Dette gøres ved at dividere disse summeringer og efterfølgende multiplicere med 100.\\ 
...
\begin{table}[H]
	...
	\begin{tabular}{cccc}
		\hline
		\rowcolor[HTML]{C0C0C0} 
		Forsøgsperson & Procent for gang & Procent for løb & Procent for cykling \\ \hline
		F1 & 41,0\% & 38,6\% & 85,5\% \\ \hline	
		F2 & 48,5\% & 51,5\% & 91,9\% \\ \hline	
		F3 & 35,9\% & 42,8\% & 85,0\% \\ \hline
		F4 & 38,7\% & 60,6\% & 91,6\% \\ \hline
	\end{tabular}
	\caption{I tabellen ses hvor mange procent den maksimale frekvensmagnitude udgør for forsøgspersonerne ved aktiviteterne gang, løb og cykling.}
	\label{tab:individuel_procent}
\end{table}\vspace{-0.5cm}

Da mængden af energi ved den maksimale magnitude er markant større for cykling i forhold til gang og løb, hvilket er illustreret i \tabref{tab:individuel_procent}, benyttes denne information til at adskille cykling fra gang og løb, ved at implementere et tærskelværdi på 70. Herved vil cykling, igennem if løkker, blive detekteret hvis den maksimale magnitude udgør 70\% eller derover, hvorimod gang og løb ikke vil blive detekteret, da disse maksimalt er henholdsvis 48,5 og 60,6\% under pilotforsøget. \\
Idet kun cykling detekteres, bliver en id kode for cykling sammen med en tidsværdi på fire sekunder, sendt til koden for brugerfladen\fxnote{vi "sender" det jo reelt set ikke, det bliver brugt i koden for GUIen med det samme...}. 
